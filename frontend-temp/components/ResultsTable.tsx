...
function RowDetail({ result }: { result: LinkResult }) {
  ...
  if (result.label === "broken" || result.label === "error") {
    suggestion = "Remove this link or replace it with a working URL.";
  } else if (result.label === "redirect" && result.final_url) {
    suggestion = `Update your link to point here instead: ${result.final_url}`;
  } else if (result.label === "dead_cta") {
    suggestion = "Add a destination URL to this button.";
  }
